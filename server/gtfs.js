...
exports.GTFS = class {
    ...
    getCalendarDates() {
        return this.db.all('SELECT * FROM calendar_dates ORDER BY service_id');
    }
    ...
    getRoutes() {
        return this.db.all('SELECT * FROM routes ORDER BY route_id');
    }

    getRouteById(route_id) {
        return new Promise((resolve, reject) => {
            this.db.get(`SELECT * FROM routes WHERE route_id = ${route_id}`, [], (err, rows)=> {
                if(err){
                    reject(err)
                } else {
                    resolve(rows)
    ...
    getStops() {
        return this.db.all('SELECT * FROM stops ORDER BY stop_id');
    }

    getStopById(stop_id) {
        return new Promise((resolve, reject) => {
            return this.db.getAsync(`SELECT * FROM stops WHERE stop_id = ${stop_id}`, [], (err, rows) => {
                if (err) {
                    reject(err)
                } else {
                    resolve(rows)
    ...
    getTrips() {
        return this.db.all('SELECT * FROM trips ORDER BY trip_id');
    }
    ...
    getNextThreeTripsForRoute(route_id, calendar_date, time) {
        return new Promise((resolve, reject) => {
            ...
            ORDER BY departure_date, departure_time
            LIMIT 3`, [route_id, calendar_date, calendar_date, time], (err, rows) => {
                    if (err) {
                        reject(err)
                    } else {
                        resolve(rows)
